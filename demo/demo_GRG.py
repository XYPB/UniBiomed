...
GRG_QUESTIONS = [
    DEFAULT_IMAGE_TOKEN + 'Could you diagnosis the abnormality in this image? Please respond with interleaved segmentation masks for the corresponding parts.',
    DEFAULT_IMAGE_TOKEN + 'Could you describe the abnormality in this image? Please highlight the region with interleaved segmentation masks.',
    DEFAULT_IMAGE_TOKEN + 'Can you generate a brief report for this image? Please output with interleaved segmentation masks for the corresponding phrases.',
    DEFAULT_IMAGE_TOKEN + 'Could you give a brief discription of what abnormality can be found within this image? Please output with interleaved segmentation masks for the corresponding phrases.',
]
...
def parse_args():
    ...
    parser.add_argument(
        '--data_path',
        default='./data/Biomed/RadGenome',
        help='save path')

    parser.add_argument(
        '--json_path',
        default='test_parse.json',
        help='save path')

    parser.add_argument(
        '--save_dir',
        default='./val_results/Grounded_Report_Generation/RadGenome',
        help='save path')
    parser.add_argument(
        '--launcher',
...
class GRGInferenceDataset:
    def __init__(self,
        ...
        for idx in json_data.keys():
            self.output_data.append(json_data[idx])
    ...
    def get_questions(self):
        import random
        question = random.choice(GRG_QUESTIONS).strip()
        return question

    def __getitem__(self, index):

        item = self.output_data[index]

        data_dict = {}

        image_file = item['image_file']
        data_dict['image_file'] = image_file.split('/')[-1]
...
def main():
    ...
    collect_results_cpu(results, len(dataset), tmpdir=args.save_dir+'_tmp')
