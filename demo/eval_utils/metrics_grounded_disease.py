...
def parse_args():
    ...
    parser.add_argument("--root", default='./data/Biomed/LGG', help="data root")
    parser.add_argument("--prediction_dir_path", default='./val_results/Grounded_disease/LGG',
...
class COCOEvalCap_GRG(COCOEvalCap):
    ...
    def evaluate(self):
        ...
        for imgId in imgIds:
            ...
            if len(temp) == len(res[imgId]):
                temp_list = []
                for idx, item in enumerate(temp):
                    new_item = res[imgId][idx].copy()

                    new_item['caption'] = item['sentences'][0]['raw'] + '.'
                    new_item['caption'] = new_item['caption'].lower()
                    del new_item['labels']
        ...
        correct_num = 0
        for key in gts.keys():
            for index in range(len(gts[key])):
                gt = gts[key][index]['caption'].lower()
                pred = res[key][index]['caption'].lower()

                total_num += 1
                if gt in pred:
                    correct_num += 1

                # if 'no findings' not in gt:
                #     total_num += 1
                #
                #     if gt in pred and 'no findings' not in gt:
                #         correct_num += 1

        Accuracy = correct_num/(total_num+1e-6)
...
def main():
    ...
    for idx, image_id in enumerate(all_id_list):
        ...
        prediction_path = f"{args.prediction_dir_path}/{file_name[:-4]}.json"
    ...
    accuracy = coco_eval.evaluate()

    # # -------------------------------#
    # # Evaluate Mask Mean dice
    coco_gt = COCO(args.gt_json_path)  # Load ground truth annotations

    output_file_path = args.prediction_dir_path + '.txt'

    with open(output_file_path, 'w') as f:
        f.write(f'Accuracy: {accuracy:.3f}\n')
        mean_dice = evaluate_mask_mdice(coco_gt, all_id_list, pred_save_path, args.root, args.split)

        f.write(f"Mean Dice (mDice) across all images: {mean_dice:.3f}\n")
