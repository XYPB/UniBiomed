...
def parse_args():
    ...
    parser.add_argument("--prediction_dir_path", default='./val_results/Grounded_Vision_Question_Answer/RadGenome',
    ...
    def evaluate(self):
        ...
        for imgId in imgIds:
            ...
            if len(temp) == len(res[imgId]) and temp[0]['caption'] is not None:
                ...
                for idx, item in enumerate(temp):
                    ...
                    new_item['caption'] = item['abnormality'] + '.'
                    del new_item['labels']
        ...
        for key in gts.keys():
            for index in range(len(gts[key])):
                gt = gts[key][index]['caption']
                pred = res[key][index]['caption']
...
def main():
    ...
    print(f"Mean Dice (mDice) across all images: {mean_dice:.3f}")

    # # -------------------------------#
    # Evaluate Recall
    # evaluate_recall_with_mapping(coco_gt, coco_cap_gt, all_id_list, pred_save_path, cap_pred_save_path,
