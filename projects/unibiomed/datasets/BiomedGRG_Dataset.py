...
def parse_annotations(example):
    ...
    orig_caption = example['report'].strip('"').strip()
    caption = example['abnormality'] + '. ' + orig_caption
...
def process_conversation(caption, anatomy):
    ...
    question = random.choice(GRG_QUESTIONS).strip().format(anatomy=anatomy)
