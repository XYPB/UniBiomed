...
from projects.unibiomed.models.lisa import LisaModel
...
class VideoLLaVASAMModel(LisaModel):
    def __init__(self,
        ...
        if not frozen_sam2_decoder:
            ...
            # memory
            self.grounding_encoder.sam2_model.memory_encoder.requires_grad_(True)
            self.grounding_encoder.sam2_model.memory_attention.requires_grad_(True)
            # prompt
    ...
    def state_dict(self, *args, **kwargs):
        ...
        # Step 4. mask decoder, prompt encoder of grounding model (SAM/SAM2)
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'mask_decoder' in k})
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'prompt_encoder' in k})
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'memory_encoder' in k})
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'memory_attention' in k})
...
class VideoLLaVASAMModel_zero3(VideoLLaVASAMModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        # print('data:', data.keys(), data['labels'].shape)
        ...
        # print('gt_masks, frames_per_batch:', len(gt_masks), gt_masks[0].shape, frames_per_batch)

        gt_masks_video = self.process_video_gt_masks(gt_masks, frames_per_batch)
        # print('gt_masks_video:', len(gt_masks_video), gt_masks_video[0].shape)

        ### Fix number of object as 5, enabling batch_size > 1 !!!!
        pred_embeddings_list_video, gt_masks_video = self.check_obj_number(
            pred_embeddings_list_video, gt_masks_video
        )
        ...
        # print('pred_masks.shape, gt_masks.shape:', pred_masks.shape, gt_masks.shape)
        # print('\n')
...
if __name__ == '__main__':
    ...
    from projects.unibiomed.models import SAM2TrainRunner
    ...
    model = VideoLLaVASAMModel_zero3(special_tokens=special_tokens,
    ...
    print(model.grounding_encoder.sam2_model)
    print('\n')

    print(model.grounding_encoder.sam2_model.sam_mask_decoder)
    print('\n')

    print(model.grounding_encoder.sam2_model.memory_encoder)
